"""Policy gateway for enterprise multi-tenant access control - WorkOS only."""
...
class PolicyContext:
    """Policy evaluation context - enterprise always requires org_id."""
    pipeline_run_id: UUID
    request_id: UUID | None
    session_id: UUID | None
    user_id: UUID
    org_id: UUID  # Required for enterprise
    service: str
    trigger: str | None
...
class PolicyGateway:
    """Enterprise policy gateway - org_id is always required."""
    ...
    async def evaluate(
        ...
        *,
        ...
        # Enterprise: policy gateway is always enabled
        ...
        # Enterprise: org_id is always required
        if context.org_id is None:
            result = PolicyResult(decision=PolicyDecision.BLOCK, reason="org_id_required")
            await self._emit_decision(checkpoint=checkpoint, context=context, result=result)
            return result

        # Enterprise: user_id is always required
        if context.user_id is None:
            result = PolicyResult(decision=PolicyDecision.BLOCK, reason="user_id_required")
            await self._emit_decision(checkpoint=checkpoint, context=context, result=result)
            return result
        ...
        # Enterprise: Verify organization membership
        async with get_session_context() as db:
        ...
        result = PolicyResult(
            ...
            details={"membership": membership_details},
        )
        await self._emit_decision(checkpoint=checkpoint, context=context, result=result)
        return result
