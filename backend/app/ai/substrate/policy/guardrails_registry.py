...
Usage:
    ...
    from app.ai.substrate.policy.guardrails_registry import register_guardrails, get_guardrails
    ...
    from app.ai.substrate.policy.guardrails_registry import get_guardrails_by_checkpoint
...
class GuardrailsRegistration:
    ...
    checkpoints: tuple[str, ...]  # Checkpoints where this runs: "pre_llm", "pre_action", "pre_persist"
...
class GuardrailsRegistry:
    ...
    def get_or_raise(cls, name: str) -> type:
        ...
        if registration is None:
            available = cls.list()
            raise KeyError(
                f"Guardrails '{name}' not found in registry. "
                f"Available guardrails: {available}"
            )
    ...
    def get_by_checkpoint(cls, checkpoint: str) -> list[type]:
        ...
        """
        return [
            reg.guardrails_class for reg in cls._registry.values()
            if checkpoint in reg.checkpoints
        ]
