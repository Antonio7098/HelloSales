...
from app.ai.substrate.agent.types import Artifact, ArtifactType, Plan
from app.ai.substrate.stages import PipelineContext, Stage, StageResult
from app.ai.substrate.tools.registry import get_tool_registry
...
class ToolExecutor(Stage):
        ...
        for action in input.actions:
            try:
                ...
                if output is None:
                    ...
                    logger.warning(
                        f"No tool available for action type: {action.type}"
                    )
                    continue
                ...
                if output.success:
                    ...
                    result.actions_failed += 1
                    logger.error(
                        f"Action {action.type} failed: {output.error}"
                    )
    ...
    async def run(
        self, ctx: PipelineContext, input: Any = None
    ) -> StageResult:
