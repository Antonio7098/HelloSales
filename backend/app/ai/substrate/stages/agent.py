...
from app.ai.substrate.observability import PipelineEventLogger
...
class ToolRegistry:
    ...
    @classmethod
    def register(cls, action_type: str, *, name: str = "", requires_approval: bool = False, description: str = "") -> Any:
...
class ToolExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            if not isinstance(result, dict):
                logger.error(
                    f"Tool '{tool.name}' returned non-dict result: {type(result)}"
                )
                return AgentResult(
                    success=False,
                    action_type=action_type,
    ...
    def get_registered_tools(self) -> dict[str, str]:
        ...
        return {
            ...
            for action_type, tool in tools.items()
        }
    ...
    if isinstance(max_payload_bytes, int) and max_payload_bytes > 0:
        for artifact in agent_output.artifacts:
            try:
                ...
                logger.error(f"Failed to calculate payload bytes for artifact {artifact.type}: {exc}", exc_info=True)
                payload_bytes = None
