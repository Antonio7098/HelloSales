...
def log_debug_failure(operation: str, error: Exception, context: dict[str, Any] | None = None) -> None:
...
def handle_provider_error(
    ...
    provider_logger.error(
        "Provider %s error during %s: %s", provider_name, operation, error, extra=extra, exc_info=True
    )
...
def handle_async_task_error(
    ...
    if context:
        extra.update(context)
    task_logger.error("Background task '%s' failed: %s", task_name, error, extra=extra, exc_info=True)
...
def with_error_handling(
    ...
    """
    from app.ai.substrate.stages.context import PipelineContext
    from app.ai.substrate.stages.graph import UnifiedStageExecutionError as StageExecutionError
...
def _is_recoverable_error(error: Exception) -> bool:
    ...
    return any(keyword in error_str for keyword in recoverable_keywords) or \
...
class StageRunner:
    ...
    async def run(
        ...
        *,
        ...
        """
        from app.ai.substrate.stages.graph import UnifiedStageExecutionError as StageExecutionError
        from app.ai.substrate.stages.result import StageResult
        ...
        try:
            ...
            data = (on_success and on_success(result)) or (result if isinstance(result, dict) else {})
            status = "completed"
