...
from .base import PipelineTimer, Stage, StageContext, StageKind, StageOutput, StageStatus
from .inputs import StageInputs, create_stage_inputs
from .ports import create_stage_ports_from_data_dict
...
class UnifiedStageGraph:
    ...
    async def _execute_node(
        ...
        logger.info(
            f"Stage {name} executing with {len(prior_outputs)} prior outputs from deps: {spec.dependencies}. "
            f"Available completed: {list(completed.keys())}"
        )
        ...
        if original_inputs is not None:
            ...
            logger.info(f"Stage {name}: got inputs from config, ports.audio_data={getattr(ports, 'audio_data', None) is not None}, ports.recording={getattr(ports, 'recording', None) is not None}")
        else:
            logger.warning(f"Stage {name}: no inputs in config, falling back to data dict. config keys={list(ctx.config.keys())}")
        ...
        # Also preserve the original data dict for stage dependencies
    ...
    async def _run_stage(self, spec: UnifiedStageSpec, ctx: StageContext) -> StageOutput:
        ...
        try:
            ...
            ctx.emit_event("stage_completed", {
                "stage": spec.name,
                "status": result.status.value,
                "duration_ms": duration_ms,
            })
