...
from app.models import Session, SessionSummary, UserMetaSummary
...
class MetaSummaryService:
    ...
    async def get_or_create(self, *, user_id: UUID) -> UserMetaSummary:
        result = await self.db.execute(
            select(UserMetaSummary).where(UserMetaSummary.user_id == user_id)
        )
        meta = result.scalar_one_or_none()
        if meta:
            return meta

        memory = MetaSummaryMemory().model_dump(mode="json")
        meta = UserMetaSummary(user_id=user_id, memory=memory, summary_text="")
        self.db.add(meta)
        await self.db.flush()
        return meta
    ...
    ) -> tuple[UserMetaSummary, list[SessionSummary]] | None:
        if max_sessions <= 0:
            return None

        try:
            meta = await self._get_or_create_for_update(user_id=user_id)
            ...
            # Latest summary per session (Postgres DISTINCT ON)
            result = await self.db.execute(
                select(SessionSummary)
                .join(SessionSummary.session)
                .where(Session.user_id == user_id)
                .distinct(SessionSummary.session_id)
                .order_by(SessionSummary.session_id, SessionSummary.version.desc())
            )
            latest_per_session = list(result.scalars().all())

            candidates = [s for s in latest_per_session if s.id not in processed]
            candidates.sort(key=lambda s: (s.created_at, s.session_id, s.version))
            to_merge = candidates[:max_sessions]
            ...
            for summary in to_merge:
                meta = await self._merge_session_summary_locked(
                    user_id=user_id,
                    meta=meta,
            ...
            logger.info(
                "Meta summary catch-up merged",
                extra={
                    "service": "meta_summary",
                    "user_id": str(user_id),
                    "merged_count": len(merged),
                    "last_session_id": str(merged[-1].session_id) if merged else None,
                    "last_session_summary_id": str(merged[-1].id) if merged else None,
                },
            )
            return meta, merged
        except Exception:
    ...
    async def _get_or_create_for_update(self, *, user_id: UUID) -> UserMetaSummary:
        result = await self.db.execute(
            select(UserMetaSummary).where(UserMetaSummary.user_id == user_id).with_for_update()
        )
        meta = result.scalar_one_or_none()
        if meta is not None:
            return meta
    ...
    async def _merge_session_summary_locked(
        self,
        *,
        user_id: UUID,
        meta: UserMetaSummary,
