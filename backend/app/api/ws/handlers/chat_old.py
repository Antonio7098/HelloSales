...
async def handle_chat_typed(
    ...
    This handler uses stageflow for a simpler pipeline without enrichers.
    """
    await _handle_chat_typed_stageflow(
        websocket=websocket,
        payload=payload,
        manager=manager,
    )
...
async def _handle_chat_typed_stageflow(
    websocket: WebSocket,
    payload: dict[str, Any],
    manager: ConnectionManager,
) -> None:
    """Handle incoming chat.typed using stageflow pipeline.

    This is a simplified handler using stageflow for basic chat without
    enrichers, assessment, or other complex features.
    """
    # Get connection info
    conn = manager.get_connection(websocket)
    if not conn:
    ...
    user_id = conn.user_id
    session_id_from_conn = conn.session_id
    org_id = getattr(conn, "org_id", None)
    ...
    # Parse message ID or generate one
    if message_id_str:
        try:
        ...
        try:
            ...
            await manager.send_message(
                websocket,
                {
                    ...
                    "payload": {
                        "code": "INVALID_PAYLOAD",
                        "message": "Invalid sessionId UUID format",
                    },
                },
            )
            return
    elif session_id_from_conn:
        session_id = session_id_from_conn
    else:
        # Create new session
        async with get_session_context() as db:
            session = Session(user_id=user_id)
            ...
            logger.info(
                "Session created on first message (stageflow)",
                extra={
                    "service": "chat",
                    "user_id": str(user_id),
                    "session_id": str(session_id),
    ...
    logger.info(
        "Processing chat message (stageflow)",
        extra={
            "service": "chat",
            "ws_id": id(websocket),
            "user_id": str(user_id),
    ...
    try:
        ...
        # Run stageflow pipeline
        runner = get_chat_pipeline_runner()
        ...
        llm_result = results.get("llm_stream")
        if llm_result and llm_result.status.value == "ok":
            ...
            logger.info(
                "Chat message handled successfully (stageflow)",
                extra={
                    "service": "chat",
                    "ws_id": id(websocket),
                    "session_id": str(session_id),
            ...
            error_msg = llm_result.error if llm_result else "Unknown error"
            raise Exception(error_msg)

    except Exception as e:
        logger.error(
            "Chat message handling failed (stageflow)",
            extra={
                "service": "chat",
                "ws_id": id(websocket),
                "session_id": str(session_id),
...
async def _get_session_messages(session_id: uuid.UUID) -> list[dict]:
