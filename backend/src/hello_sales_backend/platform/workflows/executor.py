...
from hello_sales_backend.platform.llm.contracts import ChatMessage, ChatModelPort
...
class WorkflowExecutor:
    ...
    async def run_worker_run_workflow(
        self,
        *,
        metadata: TaskMetadata,
        worker_name: str,
        ...
        """Execute one worker run through the app-owned Stageflow boundary."""
        ...
        async def execute_worker(_ctx: Any) -> dict[str, object]:
            await execute()
            return {"worker_name": worker_name}

        async def summarize(ctx: Any) -> dict[str, object]:
            stage_output = ctx.inputs.get_output("execute_worker")
            if stage_output is None:
                raise RuntimeError("Worker workflow execution stage output is missing")
            result = stage_output.data
            return {
                "workflow_name": "worker_run_workflow",
                "worker_name": result["worker_name"],
            }

        pipeline = self.runtime.pipeline_factory.create_pipeline(
            name="worker_run_workflow",
            stages=[
                WorkflowStageSpec(name="input_guard", handler=input_guard, kind=WorkflowStageKind.GUARD),
                WorkflowStageSpec(
                    name="execute_worker",
                    handler=execute_worker,
                    kind=WorkflowStageKind.WORK,
                    dependencies=("input_guard",),
                ),
                WorkflowStageSpec(
