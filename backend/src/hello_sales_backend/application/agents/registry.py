...
class AgentRegistry(AgentDefinitionResolverPort):
    ...
    def list_profiles(self) -> list[tuple[str, str]]:
        """Return registered profile metadata in stable order."""

        return [(item.agent_id, item.display_name) for item in self.definitions()]
