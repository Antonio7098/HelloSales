...
class KeywordToolSelectionPolicy:
    ...
    def select(self, user_input: str, catalog: AgentToolCatalog) -> list[AgentToolRequest]:
        ...
        if any(token in normalized for token in ("status", "runtime", "system", "health")):
        ...
        if ("diagnostic" in normalized and "job" in normalized) or "run diagnostic" in normalized:
            if catalog.has("run_diagnostic_job"):
