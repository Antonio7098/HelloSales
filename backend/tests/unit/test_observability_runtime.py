...
def test_prometheus_metrics_runtime_renders_expected_metric_families() -> None:
    ...
    assert 'hello_sales_http_requests_total{method="GET",outcome="success",route="/api/health/readiness",status_code="200"} 1.0' in metrics_text
    assert 'hello_sales_health_check_status{check="database",kind="readiness",required="false"} 1.0' in metrics_text
    assert 'hello_sales_background_tasks_completed_total{purpose="jobs.diagnostic",status="completed"} 1.0' in metrics_text
