...
@pytest.mark.asyncio
async def test_generic_agent_runtime_completes_turn_with_tool_and_provider_response():
    store = InMemoryAgentStore()
...
@pytest.mark.asyncio
async def test_generic_agent_runtime_pauses_for_approval():
    store = InMemoryAgentStore()
...
@pytest.mark.asyncio
async def test_generic_agent_runtime_records_tool_failures():
    store = InMemoryAgentStore()
    await _seed_run(store, input_text="show runtime status")
